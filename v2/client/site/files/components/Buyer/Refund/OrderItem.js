...
export default function OrderItem({item,refund}) {
  ...
  return (
    <>
                ...
                <div className="">
                    <article className="modal-container" style={{width: '100%', borderRadius: '5px'}}>
                        ...
                        <footer className="modal-container-footer" style={{justifyContent: 'space-between'}}>
                            <button className="button is-ghost" style={{background: '#FFA500'}} onClick={e=> {
        ...
        <div className="order-card-data" id={`${item?.product_id}`} style={{border: selected_order ? '2px solid #FFA500' : 'none'}}>
                ...
                <div className="body-cnt">
                    ...
                    <div className="body-cnt-mid">
                        <div className="seller">
                            <span style={{
                                whiteSpace: 'nowrap', /* Prevent text from wrapping */
                                visibility: 'hidden',    /* Hide any overflow text */
                                textOverflow: 'ellipsis'
                            }}>Seller: {item?.user_id}</span>
                        </div>

                        <div className="stock">
                            {refund?.stock} Refunding
                        </div>
                    </div>
                    
                    <div className="body-cnt-btm">
                        {
                            refund?.status?.state === 'cancelled'
                            ?
                            <>
                            ...
                            :
                            refund?.status.state === 'completed'
                            ?
                            <>
                                {/* <button style={{opacity: '1', background: 'yellowgreen'}} onClick={e=>{window.location.href = `/new-return/${item?.product_id}`}}>
                                    Return & refund
                                </button> */}
                                <button style={{height: 'auto', background: order?.status.state === 'completed' ? 'green' : '#FFA500'}} disabled>
                                    {
                                        'Completed'
                                    }
                                </button>
                            ...
                            <>
                                <button onClick={e => {
                                    ...
                                    :
                                        ...
                                        axios.post('https://cs-server-olive.vercel.app//remove-refund', {refund_id: refund?.refund_id})
                                        .then(({data})=>{
                                            console.log(data)
                                            if(data){
                                                buyer_overlay_setup(false, '');
                                ...
                                <button onClick={e=> refund?.haverefunded ? window.location.href=`/order-tracking/${item?.product_id}` : window.location.href=`/checkout/${item?.product_id}`}>
                                    {
                                        refund?.haverefunded
                                        ?
                                        <>View refund</>
