...

                {
                    ...
                    order_list?.order?.pick_up_channels.map(item=> 
                        <div className='shadow-sm' style={{display: 'flex', alignItems: 'center', fontWeight: '500', fontSize: '20', justifyContent: 'space-between', border: '1px solid #FFA500', flexDirection: 'row', padding: '5px 5px 5px 10px', width: '100%', background: '#fff', position: 'relative', borderRadius: '5px', marginBottom: '10px'}}>
                            ...
                            <button className='shadow-sm' style={{ position: 'absolute', top: '5px', right: '5px', height: 'auto', width: 'auto', display: 'flex', alignItems: 'center', justifyContents: 'center', padding: '8px', textAlign: 'center', background: '#fff', color: '#FFA500', fontSize: 'small', float: 'right', color: '#fff', background: '#FFA500', borderRadius: '5px' }} onClick={e => window.location.href = `/new-order/${pathname.split('/').splice(-1)[0]}?order_id=${order_list?.order?.order_id}`}>
                                Edit
                            </button>
                        </div>
                    )
