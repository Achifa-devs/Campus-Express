...
export async function findRefundById({ refund_id }) {
    ...
    `SELECT * FROM refund_list WHERE refund_id = $1`,
    [refund_id]
    );
  return result.rows;
};
...
// Find refunds
export async function findRefunds({ buyer_id }) {
  const result = await pool.query(
    `SELECT * FROM refund_list WHERE buyer_id = $1`,
    [buyer_id]
  );
  return result.rows;
};
...
// Create refund
export async function createRefund({ buyer_id, order_id, amount, reason }) {
  const result = await pool.query(
...
export async function deleteRefundById({ refund_id }) {
  const result = await pool.query(
    `DELETE FROM refund_list WHERE refund_id=$1`,
    [refund_id]
  );
  let response = await errorHandler(result?.rowCount);
  return response;
...
export async function confirmRefund({ refund_id }) {
  const result = await pool.query(
    `UPDATE refund_list set status="completed" WHERE refund_id=$1`,
    [refund_id]
  );
  return result.rows;
};
...
export async function cancelRefund({ refund_id }) {
  const result = await pool.query(
    `UPDATE refund_list set status="cancelled" WHERE refund_id=$1`,
    [refund_id]
  );
  return result.rows;
};