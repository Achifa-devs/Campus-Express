// import pool from "../config/db.js"

import pool from "../../config/db.js";
import shortId from "short-id"
import {
  errorHandler
} from "../../utils/erroHandler.js";

// Find shop shipping info
export async function findShopShippingInfoById({ seller_id }) {
  const result = await pool.query(
    `SELECT * FROM shipping WHERE seller_id = $1`,
    [seller_id]
  );
  return result.rows;
};


// Create new shipping info
export async function createShopShippingInfo({ Address1,Address2,Address3,Address4,City,State,Country,seller_id }) {
  
  const result = await pool.query(
    `INSERT INTO shipping (
        id, address1, address2, address3, address4, town, state, country,seller_id
    ) VALUES (
        DEFAULT, $1, $2, $3, $4, $5, $6, $7, $8
    )`,
    [
      Address1,Address2,Address3,Address4,City,State,Country,seller_id
    ]
  );
  let response = await errorHandler(result?.rowCount);
  return response;
};


// Update shop shipping info
export async function UpdateShopShippingInfo({ Address1,Address2,Address3,Address4,City,State,Country,seller_id }) {
  const result = await pool.query(
    `UPDATE shipping set address1 = $1, address2 = $2, address3 = $3, address4 = $4, town = $5, state = $6, country = $7 WHERE seller_id = $8`,
    [
      Address1,Address2,Address3,Address4,City,State,Country,seller_id
    ]
  );
  let response = await errorHandler(result?.rowCount); 
   return response;
};



// Find shop reviews by ID
export async function findShopReviewsById({ seller_id }) {
  const result = await pool.query(
    `SELECT * FROM reviews WHERE seller_id = $1`,
    [seller_id]
  );
  return result.rows;
};


// Find shop details by ID
export async function findShopDetailsById({ seller_id }) {
  const result = await pool.query(
    `SELECT * FROM campus_shops WHERE seller_id = $1`,
    [seller_id]
  );
  return result.rows;
};


// Create new shop
export async function createShop({ shop_id, seller_id, newShopName }) {
  
  const result = await pool.query(
    `INSERT INTO campus_shops (
        id,shop_id,seller_id,title,category,status,description,logo_url,open_hrs,social_links,is_verified,created_at,updated_at
    ) VALUES (
        DEFAULT, $1, $2, $3, '', 'active', '', '', '', '', ${false}, $4, $5
    )`,
    [
      shop_id, seller_id, newShopName, new Date(), new Date()
    ]
  );
  let response = await errorHandler(result?.rowCount);
  return response;
};


// Update shop 
export async function UpdateShop({ seller_id, newShopname, description }) {
  const result = await pool.query(
    `UPDATE campus_shops set title = $2, description = $3 WHERE seller_id = $1'`,
    [seller_id, newShopname, description]
  );
  let response = await errorHandler(result?.rowCount); 
   return response;
};

export async function UpdateShopCategory({ seller_id }) {
  const result = await pool.query(
    `UPDATE campus_shops set inventory='${JSON.stringify(inventory)}' WHERE seller_id = $1'`,
    [seller_id]
  );
  let response = await errorHandler(result?.rowCount); 
   return response;
};

