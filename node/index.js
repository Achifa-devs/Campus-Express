...
CAMPUSSPHERE_SERVER.get('/boosted-metrics', parser, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `
        SELECT 'views' AS source, v.id, v.user_id, u.campus, u.fname, v.date::timestamp AS created_at
        FROM views v
        ...
        SELECT 'impression' AS source, i.id, i.user_id, u.campus, u.fname, i.date::timestamp AS created_at
        FROM impression i
        ...
        SELECT 'contact_clicks' AS source, c.id, c.user_id, u.campus, u.fname, c.date::timestamp AS created_at
        ...
        SELECT 'search_appearances' AS source, s.id, s.user_id, u.campus, u.fname, s.appeared_at::timestamp AS created_at
        ...
        SELECT 'shares' AS source, sh.id, sh.user_id, u.campus, u.fname, sh.date::timestamp AS created_at
        FROM shares sh
