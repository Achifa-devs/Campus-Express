...
async function checkDuplicates(data, user_id) {
  const {
    rows
  } = await pool.query(`SELECT 1 FROM search_appearances WHERE product_id = $1 AND user_id = $2`, [data?.product_id, user_id]);
  return rows.length === 0;
}
async function insertSearchAppearance(item, user_id, word) {
  const result = await pool.query(`INSERT INTO search_appearances(id, search_id, user_id, vendor_id, product_id, query, appeared_at) 
     VALUES (DEFAULT, $1, $2, $3, $4, $5, $6)`, [shortId.generate(10), user_id, item?.user_id, item?.product_id, word, new Date()]);
  return result.rowCount > 0;
}
async function updateSearchAppearances(item) {
  const result = await pool.query(`UPDATE products SET search_appearances = search_appearances + 1 WHERE product_id = $1`, [item?.product_id]);
  return result.rowCount > 0;
}