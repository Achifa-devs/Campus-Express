...
export const postShopView = async payload => {
  ...
  try {
    // Business logic
    let existingView = await findShopViewId({
      shop_id,
      user_id
    });
    ...
    let newView = await createShopView({
      user_id,
      shop_id
    });
    if (newView < 1) {
    ...
    let response = await updateShopView({
      shop_id
    });
    return response;
  } catch (error) {
...
export const postContactClick = async payload => {
  ...
  try {
    // Business logic
    let existingView = await findContactClickId({
      product_id,
      user_id
    });
    if (existingView.length > 0) {
    ...
    let newView = await createContactClick({
      user_id,
      product_id
    });
    if (newView < 1) {
    ...
    let response = await updateContactClick({
      product_id
    });
    return response;
  } catch (error) {
...
export const postShare = async payload => {
  ...
  try {
    // Business logic
    let existingView = await findShareId({
      product_id,
      user_id
    });
    if (existingView.length > 0) {
    ...
    let response = await updateShare({
      product_id
    });
    return response;
  } catch (error) {
...
export const postImpression = async payload => {
  ...
  try {
    // Business logic
    let existingView = await findImpressionId({
      product_id,
      user_id
    });
    if (existingView.length > 0) {
    ...
    let newView = await createImpression({
      user_id,
      product_id
    });
    if (newView < 1) {
    ...
    let response = await updateImpression({
      product_id
    });
    return response;
  } catch (error) {
...
export const postUpdateContactClickForUnknownCustomer = async payload => {
  ...
  const response = await updateContactClickForUnkownBuyer({
    unknown_user_id,
    registered_id
  });
  return response;
