...
export async function findProducts({
  ...
  if (limit !== null) {
    const result = await pool.query(`SELECT * FROM products WHERE state->>'state' = 'active' LIMIT $1`, [limit]);
    return result.rows;
  } else {
    const result = await pool.query(`SELECT * FROM products WHERE state->>'state' = 'active'`);
    return result.rows;
  }
}
;
export async function findProductsType({
  ...
  const result = await pool.query(`SELECT * FROM products WHERE state->>'state' = 'active' AND others->>'cType' = $1 LIMIT $2`, [type, limit]);
  return result.rows;
}
;
...
export async function findProductsByCategory({
  ...
  const result = await pool.query(`SELECT * FROM products WHERE category = $1 AND state->>'state' = 'active'LIMIT $2`, [category, limit]);
  return result.rows;
}
;
...
export async function findProductsByCategoryAndGender({
  ...
  const result = await pool.query(`SELECT * FROM products WHERE category = $1 AND state->>'state' = 'active' AND others->>'gender' = $2 LIMIT $3`, [category, cap_gender, limit]);
  return result.rows;
}
;
