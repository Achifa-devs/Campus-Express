...
export async function findFavourites({
  ...
  const result = await pool.query(`SELECT * FROM favourite WHERE buyer_id = $1`, [buyer_id]);
  return result.rows;
}
;
...
export async function deleteFavourite({
  ...
  const result = await pool.query(`DELETE FROM favourite WHERE buyer_id=$1 AND product_id=$2`, [buyer_id, product_id]);
  let response = await errorHandler(result?.rowCount);
  return response;
}
;