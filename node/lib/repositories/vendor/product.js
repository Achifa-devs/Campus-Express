...
export async function findProductById({
  ...
  const result = await pool.query(`SELECT * FROM products WHERE product_id = $1 AND user_id = $2`, [product_id, user_id]);
...
export async function createProduct({
  ...
  try {
    ...
    const result = await pool.query(`INSERT INTO products(
      ...
      )`, [product_id, user_id, 'unsold', replacedTitle, replacedDescription, price || 0, false, category, JSON.stringify(dynamicData), date, JSON.stringify({
      state: "active",
      reason: "rent paid"
    }), 0, 0, stock || 0, thumbnail_id, shipping_data?.shipping_policy, JSON.stringify(shipping_data?.shipping_range), shipping_data?.shipping_duration, campus, state, thumbnail_public_id, purpose, 0, 0, 0]);
...
export async function deleteProductPromotion({
  ...
  const result = await pool.query(`DELETE FROM promotions WHERE product_id = $1`, [product_id]);
  return;
}
...
export async function updateProduct({
  ...
  const result = await pool.query(`UPDATE products SET 
     ...
     WHERE product_id = $20 AND user_id = $21`, ['unsold', replacedTitle, replacedDescription, price || 0, 0, category, JSON.stringify(dynamicData), date, JSON.stringify({
    state: "active",
    reason: "rent paid"
  }), 0, 0, stock || 0, thumbnail_id, shipping_data?.shipping_policy, JSON.stringify(shipping_data?.shipping_range), shipping_data?.shipping_duration, campus, state, thumbnail_public_id, product_id,
  // ✅ aligned
  user_id]);
...
export async function updateAdQuota({
  user_id
}) {
  const query = `
    UPDATE subscriptions 
