...
export async function findProductById({
  ...
  const result = await pool.query(`SELECT * FROM products WHERE product_id = $1 AND user_id = '${user_id}'`, [product_id, user_id]);
  return result.rows;
}
;
...
export async function createProduct({
  ...
  const result = await pool.query(`INSERT INTO products(
        ...
        )`, [product_id, user_id, 'unsold', replacedTitle, replacedDescription, price, 0, category, JSON.stringify(dynamicData), date, JSON.stringify({
    "state": "active",
    "reason": "rent paid"
  }), 0, 0, stock, thumbnail_id, shipping_data?.shipping_policy, JSON.stringify(shipping_data?.shipping_range), shipping_data?.shipping_duration, campus, state, thumbnail_public_id]);
...
export async function updateProduct({
  ...
  const result = await pool.query(`UPDATE products set 
        ...
        `, ['unsold', replacedTitle, replacedDescription, price, 0, category, JSON.stringify(dynamicData), date, JSON.stringify({
    "state": "active",
    "reason": "rent paid"
  }), 0, 0, stock, thumbnail_id, shipping_data?.shipping_policy, JSON.stringify(shipping_data?.shipping_range), shipping_data?.shipping_duration, campus, state, thumbnail_public_id, productId, user_id]);
  let response = await errorHandler(result?.rowCount);
  return response;
}
;