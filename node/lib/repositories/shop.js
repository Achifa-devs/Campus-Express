...
export async function findShopReviewsById({
  ...
  const result = await pool.query(`SELECT * FROM reviews WHERE seller_id = $1`, [seller_id]);
  return result.rows;
}
;
...
export async function findShopDetailsById({
  ...
  const result = await pool.query(`SELECT * FROM campus_shops WHERE seller_id = $1`, [seller_id]);
  return result.rows;
}
;

// Find shop content by ID
export async function findShopContentById({
  seller_id
}) {
  const result = await pool.query(`SELECT * FROM seller_shop WHERE seller_id = $1`, [seller_id]);
  return result.rows;
}
;
