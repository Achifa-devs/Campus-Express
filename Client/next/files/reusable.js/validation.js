...
function selects(item, name, element) {
    let check = item.length < 1 ? {err: `product ${name} cannot be empty`, success: false, name: name, element: element}  :  {err: '', success: true, name: name, element: element}
    return check;
}
...
function inputs(item, name, element, file) {
    if(name==='stock'){
        ...
        let check = file.length < 1 ? {err: `sorry ${name} samples cannot be empty`, success: false, name: name, element: element}  : file.length < 1 ? {err: `sorry ${name} samples must be at least 1`, success: false, name: name, element: element} : {err: ``, success: true, name: name, element: element}
        return check;
    } 
}
