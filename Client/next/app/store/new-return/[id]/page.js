...
export default function NewOrder() {
    ...
    useEffect(() => {
        ...
        try {
            axios.get('https://cs-node.vercel.app/product', {params: {id: pathname.split('/').splice(-1)}})
            .then(({data})=>{
                setItem(data[0])
    ...
    useEffect(() => {
        ...
        if (user_id !== '' && user_id !== null && user_id !== 'null') {
            ...
            axios.get('https://cs-node.vercel.app/refund', {params: {user_id: user_id, product_id: pathname.split('/').splice(-1)[0]}})
            .then(({data})=>{
                if (data.length > 0) {
                    set_refund(data)
