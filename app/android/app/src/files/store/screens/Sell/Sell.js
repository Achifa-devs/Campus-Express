...
export default function Sell() {
  ...
  useEffect(() => {
    ...
    (async function getUser(params) {
      ...
      let res = await fetch(`http://192.168.209.146:9090/vendor/shop?user_id=${id}`)
  ...
  const deleteFromServer = async (url) => {
    try {
      setIsLoading(true);
      const response = await axios.post('http://192.168.209.146:9090/delete', {
        url
      });
  ...
  const handleSubmit = () => {
    ...
    setIsLoading(true)
    fetch(`http://192.168.209.146:9090/vendor/create-shop`, {
      method: 'post',
      headers: {
