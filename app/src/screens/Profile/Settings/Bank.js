...
export default function Bank() {
    ...
    function verify() {
        fetch(`https://estate-dun-eta.vercel.app/bank-verification`, {
            method: 'post',
            headers: {
    ...
    function create() {
        fetch(`https://estate-dun-eta.vercel.app:9090/api/shop/payment/create`, {
            method: 'post',
            headers: {
                "Content-Type": "Application/json"
            },
            body: JSON.stringify({acct_num: acctNo, beneficiary: beneficiary, bank: bankName, userid: user?.userid })
        })   
        .then(async(result) => {
            let response = await result.json();
            if (response.success) {
        ...
        <ScrollView style={{height: '100%', width: '100%', padding: 10, backgroundColor: '#fff'}}>
            ...
            <View>
                ...
                <View style={{ height: 80, display: 'flex', color: '#000', width: '100%', flexDirection: 'column', marginBottom: 10}}>
                    ...
                    <TextInput 
 placeholderTextColor={"#333333"} style={{height: 50, fontFamily: 'Roboto', padding: 10, borderRadius: 5, marginBottom: 2, width: '100%',  backgroundColor: '#efefef'}} maxLength={10} keyboardType='numeric' name="account-number" onChangeText={txt=> setAcctNo(txt)}  placeholder="Account number"  />
