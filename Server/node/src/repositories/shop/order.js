...
// Find order by ID
export async function findOrderById({ product_id, user_id }) {
    const result = await pool.query(
    `SELECT * FROM orders WHERE product_id = $1 AND user_id = $2`,
    [product_id, user_id]
    );
  return result.rows;
};
...
// Confirm order
export async function confirmOrder({ order_id,user_id,product_id }) {
  const result = await pool.query(
...
// deleteOrder
export async function deleteOrder({ user_id, product_id }) {
  const result = await pool.query(
...
export async function createOrderWithId({ user_id, product_id, stock, price, locale, order_id }) {
  const result = await pool.query(
    `INSERT INTO orders(
        id, order_id, product_id, status, date, stock, user_id, price, pick_up_channels, havePaid
    ) VALUES (
