...
export default function NewOrderSummary({item,stock,deliveryOpt,refund_id}) {
    ...
    async function handleNewRefund() {
        ...
        if(deliveryOpt !== -1){
            if (deliveryOpt === 0 && check_2.length > 0 || deliveryOpt === 1 && check_1.length > 0) {
                ...
                fetch('https://ce-server.vercel.app/create-refund', {
                    method: 'post',
                    headers: {
    ...
    async function handleRefundUpdate() {
        ...
        if(deliveryOpt !== -1){
            if(deliveryOpt === 0 && check_2.length>0 || deliveryOpt === 1 && check_1.length>0){
                fetch('https://ce-server.vercel.app/update-refund', {
                    method: 'post',
                    headers: {
                        "Content-Type": "Application/json"
                    },
                    body: JSON.stringify({
                        buyer: buyer_id, product_id: item.product_id, price: parseInt(item.price)*parseInt(stock), stock: stock, locale: pickup_channel, refund_id: refund_id
                    })
                })
                .then(async(result)=> {
                    let response = await result.json()
