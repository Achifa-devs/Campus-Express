...
export default function DeliveryAddress({item,refund_id,refund,updateDeliveryOpt}) {
    ...
    // let [refund_list, set_refund_list] = useState('');


    useEffect(() => {
      if(refund !== ''){
        if(refund[0]?.refund?.pick_up_channels[0]?.channel === 'Custom Location Pickup'){
          document.querySelector('.delivery-0').checked = true;
        }else{
          document.querySelector('.delivery-1').checked = true;
        }
        setLocale([{channel: refund[0]?.refund?.pick_up_channels[0]?.channel, locale: refund[0]?.refund?.pick_up_channels[0]?.locale, date: refund[0]?.refund?.pick_up_channels[0]?.date, index: locale.length}])
        setpickUpChannel(refund[0]?.refund?.pick_up_channels[0]?.channel)
        setdeliveryOpt(refund[0]?.refund?.pick_up_channels[0]?.channel === 'Custom Location Pickup' ? 0 : 1)
      }
    }, [refund])
  ...
  return (
    <>
      <div className="pickup-overlay">
        <PickupChannel updateLocation={updateLocation} title={pickUpChannel} edit={false} refund_data={''} />
      </div>
        ...
        <div className="delivery-pick-up-station" >
          ...
          <div className='' style={{display: 'flex', alignItems: 'center', fontWeight: '500', fontSize: '20', justifyContent: 'space-between', brefund: '1px solid #FF4500', flexDirection: 'row', padding: '0', width: '100%', background: '#fff', position: 'relative', brefundRadius: '5px', marginBottom: '10px', brefund: 'none'}}>
            ...
            <div style={{padding: '0', width: '100%'}}>
              ...
              <div className='shadow-sm' style={{display: 'flex', alignItems: 'flex-start', fontWeight: '500', fontSize: '20', justifyContent: 'flex-start', flexDirection: 'column', position: 'relative', brefund: 'none', padding: '10px'}}>
                ...
                <div style={{
                  ...
                  display: 'flex', alignItems: 'flex-start', fontWeight: '500', fontSize: '20', justifyContent: 'flex-start', flexDirection: 'column', position: 'relative', brefund: 'none', padding: '10px'
                }}>
                  <section style={{padding: '10px', fontSize: '12', fontWeight: '400'}}>
                    <small style={{fontSize: 'small'}}>
                    You Will Submit The Item At The Designated Shop Provided By The Seller 
                    </small>
                  </section>
                
                
                  <section  style={{display: 'flex', alignItems: 'flex-start', fontWeight: '500', fontSize: '20', justifyContent: 'flex-start', flexDirection: 'column', padding: '10px', width: '100%', background: '#fff', position: 'relative', brefundRadius: '5px'}}>
                    {
                      locale.map((item) => 
                        ...
                        <div className='shadow-sm' style={{display: 'flex', alignItems: 'center', fontWeight: '500', fontSize: '20', justifyContent: 'space-between', brefund: '1px solid #FF4500', flexDirection: 'row', padding: '5px 5px 5px 10px', width: '100%', background: '#fff', position: 'relative', brefundRadius: '5px', marginBottom: '10px'}}>
                          ...
                          <button  onClick={e=>deleteLocation(item.index)} style={{width: '35px', height: '35px', padding: '5px', textAlign: 'center', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
                            <img src={'../../../assets/delete-svgrepo-com (1).svg'} style={{height: '100%', margin: '0', left: 'unset', float: 'unset', width: '100%', position: 'relative'}} alt="" />
                          </button>
                        </div>
                        :
                        ''
              ...
              <div className='shadow-sm' style={{display: 'flex', alignItems: 'flex-start', fontWeight: '500', fontSize: '20', justifyContent: 'flex-start', flexDirection: 'column', position: 'relative', brefund: 'none', padding: '10px'}}>
                ...
                <section style={{display: 'flex', alignItems: 'center', fontWeight: '500', fontSize: '20', justifyContent: 'flex-start', flexDirection: 'row-reverse', width: '80%'}}>
                  <label style={{height: '20px', padding: '0', width: '100%', display: 'flex', alignItems: 'flex-end', fontSize: 'small'}} htmlFor="">Door Step Return</label>
                  &nbsp;&nbsp;
                ...
                <div style={{
                  ...
                  <section style={{padding: '10px', fontSize: '12', fontWeight: '400'}}>
                      <small style={{fontSize: 'small'}}>
                        The Item Will Be Picked At Your Door Step (Charges Applies)
                      </small>
                  </section>
...
function PickupChannel({updateLocation,title,edit,refund_data}) {
    ...
    function addLocation() {
      ...
      let res = ({locale: [state,city,address1,address2,address3,address4].map(item=>item).join(', '), date: {yr: new Date().getFullYear(), mth: selectedMonth, dy: day}})
      updateLocation(res)
      rmLocation()
              ...
              <h6 style={{padding: '10px', color: '#FF4500', fontWeight: '500'}}><u>Input Your {title} Location </u></h6>
                <section style={{padding: '10px', overflow: 'auto', height: 'calc(100% - 100px)'}}>
                  ...
                  <div className="input-cnt" style={{width: '100%'}}>
                    <label style={{fontSize: 'small'}} htmlFor="">Town</label>
                  ...
                  <div className="input-cnt" style={{width: '100%'}}>
                    <label style={{fontSize: 'small'}} htmlFor="">City/Region</label>
                  ...
                  <div className="input-cnt" style={{width: '100%'}}>
                      <label style={{fontSize: 'small'}} htmlFor="">Address 1(Town)</label>
                      <input style={{width: '100%'}} onInput={e => setAddress1(e.target.value)} defaultValue={edit?.locale?.split(',')[3]} type="text" name="" id="" />
                  </div>

                  <div className="input-cnt" style={{width: '100%'}}>
                      <label style={{fontSize: 'small'}} htmlFor="">Address 2(Street Name)</label>
