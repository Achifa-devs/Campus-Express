...
function EditProfile({ data, Validation }) {
  ...
  function update_buyer() {
    ...
    if (fname !== '' && lname !== '' && gender !== '' && fname.length > 3 && lname.length > 3) {
      ...
      axios.post('https://ce-server.vercel.app/profile-update', {fname,lname,gender,buyer_id})
      .then(({data})=>{
...
function Edit({ edit, data }) {
  ...
  function update_() {
    ...
    if (edit.toLowerCase() === 'email' && email !== '' && emailRegex.test(email) || edit.toLowerCase().split(' ')[1] === 'number' && phone !== '' && phone.length >= 10 ) {
      ...
      axios.post(`https://ce-server.vercel.app/${(edit.toLowerCase().split(' ')[0])}-update`, body)
      .then(({data})=>{
        window.location.reload()
        buyer_overlay_setup(false, '')
...
function Password() {
    ...
    if (oldpwd.length >= 8) {
      if (pwd.length >= 8) {
        if (pwd === cpwd) {
          ...
          axios.post(`https://ce-server.vercel.app/password-alter`, {pwd,buyer_id,oldpwd})
          .then(({data})=>{
            console.log(data)
  ...
  return (
    <>
      ...
      <h6 style={{width: '100%', textAlign: 'center'}} className='err-mssg'>{ err}</h6>
      <section>
        <div className="input-cnt">
