...
function selects(item, name, element) {
    let check = item.length < 1 ? {err: `${name} cannot be empty`, bool: false, name: name, element: element}  :  {err: '', bool: true, name: name, element: element}
    return check;
}
...
function inputs(item, name, element, file) {
    if(name==='stock'){
        ...
        // let check = file.length < 1 ? {err: `sorry sample images cannot be empty`, bool: false, name: name, element: element} : {err: ``, bool: true, name: name, element: element}
        return {err: ``, bool: true, name: name, element: element};
    }
    else {
        let check = item.length < 1 ? {err: `sorry ${name} cannot be empty`, bool: false, name: name, element: element}  : item < 1 ? {err: `sorry stock can"t be less than one`, bool: false, name: name, element: element} : {err: ``, bool: true, name: name, element: element}
        return check;
