...
import { GetItems, GetSavedItem } from '@/app/api/buyer/get'
...
const Dashboard = () => {
  ...
  let {
    ...
    async function fetchData(overlay,category) {
      ...
      axios.get(`https://ce-server.vercel.app`, {
        params: { category: category},
        headers: headers
      })
      .then(async({data})=>{
        console.log('filtered_response: ', data.data)
        
        if(data){
          
          let filtered_response = insertFilterTag(data.data)
  ...
  useEffect(() => {
    ...
    if(buyer_info){
      GetSavedItem(buyer_info?.buyer_id)
      .then((result) => {
          console.log(result)
          if(result){
            dispatch(setSaveTo(result))
