...
function inputs(item, name, element, file) {
    if(name==='stock'){
        ...
        let check = file.length < 1 ? {err: `sorry samples/main image cannot be empty`, bool: false, name: name, element: element}  : file.length < 1 ? {err: `sorry samples/main image must be at least 1`, bool: false, name: name, element: element} : {err: ``, bool: true, name: name, element: element}
        return check;
    } 
}
